...
#皮肤边界草图提取
ret, skin_thresh = cv2.threshold(original_img_gray, 20, 255, cv2.THRESH_BINARY)
...
for k in range(30):
    ...
    cv2.line(spin_2, (50, average_1-8*k + 30), (average_2, average_1), (0, 0, 255), 1)
    cv_show('spin_2', spin_2)
    #找出交点，并计算两交点之间的距离，如果满足某条件，跳出循环
    mask_1 = np.zeros(spin_2.shape, np.uint8)
    cv2.line(mask_1, (50, average_1-8*k + 30), (average_2, average_1), (255, 255, 255), 1)
    #cv_show('mask', mask_1)
...
for i in range(mask_3.shape[0]):
    for j in range(mask_3.shape[1]):
        if top_im[i, j] and mask_3_im[i, j] == 1:
            skin_inter_points_list2.append([i, j])
print(skin_inter_points_list2)
    # print(len(point))
skin_intersection_point_2 = skin_inter_points_list2[-1]
skin_intersection_point_3 = skin_inter_points_list2[-2]
#print('F = ', skin_intersection_point_3)
# intersection_point_1_list.append(intersection_point_1)
#print(skin_intersection_point_2)


#通过交点1向椎体中心对称线做垂线，并导出交点，用以获得旁开棘突距离
slope_3 = -1 / slope_1
#print(slope_3)
...
#找出左边第一条直线与椎体变边界的交点并通过交点做与椎体中心线平行的线，寻找其与皮肤的交点
...
#print(skin_inter_points_list3)
    # print(len(point))
skin_intersection_point_4 = skin_inter_points_list3[-1]
#print('G = ', skin_intersection_point_4)
