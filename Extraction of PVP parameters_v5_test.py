...
def getDist_P2L(PointP, Pointa, Pointb):
    ...
    """
    # 求直线方程
    A = (-Pointb[0] - (-Pointa[0])) / (Pointb[1] - Pointa[1])
    B = -1
...
end_sum_1 = 0
end_count_1 = 0
for i in range(len(para_outer_index)):
    if para_outer_index[i][0] == para_outer_index[-1][0]:
        end_sum_1 = end_sum_1 + para_outer_index[i][1]
        end_count_1 += 1
    else:
        continue
end_avg_1 = end_sum_1 // end_count_1
end_point_outer = [para_outer_index[-1][0], end_avg_1]
...
cv2.line(spin_3, (end_point[1], end_point[0]), (end_point_outer[1], end_point_outer[0]), (0, 0, 255), 1)
slope_1 = ((-end_point_outer[0]) - (-end_point[0])) / (end_point_outer[1] - end_point[1] + 0.01)
b1 = -end_point_outer[0] - slope_1 * end_point_outer[1]
