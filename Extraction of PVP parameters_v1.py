...
#读取图像并转化为灰度图
original_img = cv2.imread('E:\jupyter\opencv\project\Extraction of PVP parameters\original_images\example_1.jpg')
...
ret, Outer_contour_thresh = cv2.threshold(original_roi, 170, 255, cv2.THRESH_BINARY)
#cv_show('thresh', Outer_contour_thresh)
# 均值滤波
blur = cv2.blur(Outer_contour_thresh, (3, 3))
#cv_show('blur', blur)
#提取轮廓信息
original_binary, original_contours, original_hierarchy = cv2.findContours(blur, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
#画出轮廓
...
for i in range(len(connect_contours)):
    ...
    #设定保留轮廓的条件为轮廓周长小于240
    if connect_perimeter_list[-2] > connect_length > connect_perimeter_list[-1] - 2:
...
#提取椎体前缘顶点
top_ima = Image.open('E:\jupyter\opencv\project\Extraction of PVP parameters\spin_images\spin_1.png')
...
top_sum = 0
while i <= 50:
    if top_index[i][0] == top_index[i+1][0]:
        top_sum = top_sum + top_index[i][1]
        i += 1
    else:
        break
avg = top_sum // i+1
#print(avg)
top = [top_index[0][0], avg]
#print(top)
...
for k in range(30):
    spin_2 = spin_1.copy()
    cv2.line(spin_2, (0, average_1-5*k + 30), (average_2, average_1), (0, 0, 255), 1)
    #cv_show('spin_2', spin_2)
    #找出交点，并计算两交点之间的距离，如果满足某条件，跳出循环
    mask_1 = np.zeros(spin_2.shape, np.uint8)
    cv2.line(mask_1, (0, average_1-5*k + 30), (average_2, average_1), (255, 255, 255), 1)
    #cv_show('mask', mask_1)
...
#通过关键点画线，获取直线表达式
spin_3 = spin_1.copy()
cv2.line(spin_3, (top[1], top[0]), (average_2, average_1), (0, 0, 255), 1)
cv2.line(spin_3, (top[1], top[0]), (mid_point[1], mid_point[0]), (0, 0, 255), 1)
#cv_show('spin_3', spin_3)
#计算直线斜率
slope_1 = ((-mid_point[0]) - (-top[0])) / (mid_point[1] - top[1])
slope_2 = ((-average_1) - (-top[0])) / (average_2 - top[1] + 0.01)
#print(slope_1, slope_2)
#计算直线截距
b1 = -mid_point[0] - slope_1 * mid_point[1]
b2 = -average_1 - slope_2 * average_2
#print(b1, b2)
#延长直线至图像底端
pre_end_point_1 = (-512 - b1) // slope_1
pre_end_point_2 = (-512 - b2) // slope_2
...
#画出结果
spin_4 = spin_1.copy()
cv2.line(spin_4, (top[1], top[0]), (end_point_1, 512), (0, 0, 255), 1)
cv2.line(spin_4, (top[1], top[0]), (end_point_2, 512), (0, 0, 255), 1)
#cv_show('spin_4', spin_4)

# 寻找两直线与皮肤边界的交点
mask_2 = np.zeros(spin_4.shape, np.uint8)
cv2.line(mask_2, (top[1], top[0]), (end_point_1, 512), (255, 255, 255), 1)
#cv_show('mask', mask_2)
...
# intersection_point_2 = point[-1]
# intersection_point_2_list.append(intersection_point_2)

mask_3 = np.zeros(spin_4.shape, np.uint8)
cv2.line(mask_3, (top[1], top[0]), (end_point_2, 512), (255, 255, 255), 1)
#cv_show('mask', mask_3)
...
level = int((-skin_intersection_point_1[0] - b2) // slope_2)
level_point = [skin_intersection_point_1[0], level]
#print(level_point)
#绘制结果
spin_5 = cv2.line(spin_4, (skin_intersection_point_1[1], skin_intersection_point_1[0]), (level, skin_intersection_point_1[0]), (0, 0, 255), 1)
#cv_show('spin_4', spin_4)
...
finall = cv2.addWeighted(target_1, 1, original_img, 0.7, 0)
cv_show('finall', finall)
cv2.imwrite('finall.png', finall)

#参数计算过程
parameter_AC = math.sqrt((abs(top[1] - skin_intersection_point_1[1]) ** 2) + (abs(top[0] - skin_intersection_point_1[0]) ** 2))
parameter_AD = math.sqrt((abs(top[1] - skin_intersection_point_2[1]) ** 2) + (abs(top[0] - skin_intersection_point_2[0]) ** 2))
parameter_AE = math.sqrt((abs(top[1] - level_point[1]) ** 2) + (abs(top[0] - level_point[0]) ** 2))
parameter_CE = math.sqrt((abs(skin_intersection_point_1[1] - level_point[1]) ** 2) + (abs(skin_intersection_point_1[0] - level_point[0]) ** 2))
