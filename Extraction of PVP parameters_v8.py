...
#读取图像并转化为灰度图
original_img = cv2.imread('E:\jupyter\opencv\project\Extraction of PVP parameters\original_images\example_3.jpg')
...
ret, median = cv2.threshold(original_roi, 155, 255, cv2.THRESH_BINARY)
...
cv2.line(spin_3, (end_point[1], end_point[0]), (maxDistPt[0], maxDistPt[1]), (0, 0, 255), 1)
slope_1 = ((-maxDistPt[1]) - (-end_point[0])) / (maxDistPt[0] - end_point[1] + 0.01)
...
for k in range(30):
    ...
    # 打印出像素为1的像素点的索引
    top_index_1 = []
    for i in range(result_im.shape[1]):
        for j in range(result_im.shape[0]):
            if result_im[j, i] == 1:
                top_index_1.append([j, i])
    centrum_points = []
